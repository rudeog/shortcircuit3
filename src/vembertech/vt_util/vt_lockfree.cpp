...
#if WINDOWS
// windows doesn't have aligned_alloc even in c++17
#include <malloc.h>
#else
#include <cstdlib>
#endif
...
#else
    data = std::aligned_alloc(16, Entries * EntrySize);
#endif
...
void vt_LockFree::WriteBlock(void *srcData)
{
    ...
    if (tmp >= NumEntries)
        tmp = 0;
    // ATTENTION! WritePos must absolutely not be written to the memory with the wrong value in an intermediate step
